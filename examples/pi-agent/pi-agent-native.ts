 ...
 *
 * Wires `@introspection-sdk/introspection-pi-agent` onto a `pi-agent-core` Agent
...
async function main() {
  ...
  const processor = new IntrospectionSpanProcessor({
    serviceName: "pi-agent-native-example",
  });
  const provider = new NodeTracerProvider({ spanProcessors: [processor] });
  provider.register();

  const tracer = trace.getTracer("pi-agent-native-example");
