...
function serializeWithCap(
  ...
  if (compactPayload && byteLength(compactPayload) <= MAX_BYTES) {
    console.warn(
      `[introspection-pi-agent] Compacting oversized ${attributeName} (${byteLength(fullPayload)} bytes)`,
    );
    return compactPayload;
  }

  console.warn(
    `[introspection-pi-agent] Dropping oversized ${attributeName} (${byteLength(fullPayload)} bytes)`,
  );
  return undefined;
}
