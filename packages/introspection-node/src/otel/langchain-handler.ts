...
export class IntrospectionCallbackHandler extends BaseCallbackHandler {
  ...
  private _runRoots: Map<string, string> = new Map();
  ...
  private _rootRunId(runId: string, parentRunId?: string): string {
    if (!parentRunId) return runId;
  ...
  private _endRootIfComplete(runId: string): void {
  ...
  private _createChildSpan(
    ...
    // Baggage wins so per-call agent identity (e.g. via withAgent())
    // overrides the parent-span-name walk used as a default.
    const baggageAgentName = this._readBaggage("gen_ai.agent.name");
    const agentName = baggageAgentName || this._findAgentName(parentRunId);
    if (agentName) {
      span.setAttribute("gen_ai.agent.name", agentName);
    }
    const baggageAgentId = this._readBaggage("gen_ai.agent.id");
    if (baggageAgentId) {
  ...
  handleLLMEnd(output: LLMResult, runId: string): void {
    ...
    if (tokenUsage) {
      ...
      const cacheWriteTokens =
        ...
        ((cacheCreationObj?.["ephemeral_5m_input_tokens"] as number) || 0) +
          ((cacheCreationObj?.["ephemeral_1h_input_tokens"] as number) || 0) ||
        undefined;
  ...
  private _getConversationId(metadata?: Record<string, unknown>): string {
    ...
    const baggageConvId = this._readBaggage("gen_ai.conversation.id");
    if (baggageConvId) return baggageConvId;
    return this._conversationId;
  }

  /**
   * Read a single value from the active OTel baggage. Returns undefined when
   * no propagator is installed or the key is absent.
   *
   * Used so callers can scope LangChain calls with
   * `IntrospectionClient.withAgent()` / `.withConversation()` /
   * `.withGenAi()` without threading metadata through every invoke().
   */
  private _readBaggage(key: string): string | undefined {
