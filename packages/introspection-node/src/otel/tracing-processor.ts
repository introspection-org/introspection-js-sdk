...
export interface IntrospectionTracingProcessorOptions {
  ...
  token?: string;
  /** Base URL for the API (env: INTROSPECTION_BASE_OTEL_URL, default: "https://otel.introspection.dev") */
  baseUrl?: string;
...
export class IntrospectionTracingProcessor implements TracingProcessor {
  ...
  async onTraceStart(trace: Trace): Promise<void> {
    ...
    if (trace) {
      // Use groupId as conversationId when provided so callers can link
      // multiple traces (e.g. multi-turn orchestrator runs) into one conversation.
      const conversationId =
        trace.groupId ?? `intro_conv_${randomUUID().replace(/-/g, "")}`;
  ...
  private _processResponseSpan(
    ...
    if (usage) {
      ...
      // OpenAI returns cached token count in input_tokens_details.cached_tokens
   ...
   * Handles both structured GenerationUsageData (spanData.usage) and raw
   * Chat Completions API responses stored in spanData.output[0] by
  ...
  private _processGenerationSpan(
    ...
    // GenerationUsageData uses snake_case: input_tokens / output_tokens.
    if (spanData.usage?.input_tokens) {
      otelSpan.setAttribute(
        "gen_ai.usage.input_tokens",
        spanData.usage.input_tokens,
      );
    }
    if (spanData.usage?.output_tokens) {
      otelSpan.setAttribute(
        "gen_ai.usage.output_tokens",
    ...
    if (!spanData.usage) {
      ...
      // Resolved model name (e.g. "gpt-4o-mini-2024-07-18" vs "gpt-4o-mini").
      ...
      if (rawUsage) {
        ...
        if (typeof promptTokens === "number") {
          otelSpan.setAttribute("gen_ai.usage.input_tokens", promptTokens);
        }
    ...
    // For non-Chat-Completions generation spans that do have usage set,
    // still export output messages in the standard format if present.
    if (spanData.usage && spanData.output) {
      otelSpan.setAttribute(
        "gen_ai.output.messages",
