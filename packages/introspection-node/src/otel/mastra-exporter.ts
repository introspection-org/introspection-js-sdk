...
export class IntrospectionMastraExporter extends BaseExporter {
  ...
  private _exportAgentRun(span: AnyExportedSpan): void {
    ...
    // Prefer system messages from span.input (full prompt sent to LLM) over
    // attrs.instructions (may be just the short placeholder string).
    const inputMsgs = Array.isArray(span.input)
      ? (span.input as Record<string, unknown>[])
      : undefined;
    const systemMsgsFromInput = inputMsgs?.filter((m) => m.role === "system");
    if (systemMsgsFromInput && systemMsgsFromInput.length > 0) {
      rootSpan.setAttribute(
        "gen_ai.system_instructions",
        JSON.stringify(
          systemMsgsFromInput.map((m) => ({
            type: "text",
      ...
      attrs?.["instructions"] &&
      typeof attrs["instructions"] === "string"
    ) {
      rootSpan.setAttribute(
        "gen_ai.system_instructions",
  ...
  private _exportModelStep(span: AnyExportedSpan): void {
    ...
    // Input messages — span.input is either:
    //   (a) a flat normalized array { role, content }[] from Mastra's inputMessages path, or
    //   (b) an object { body: { input/messages, tools } } from the raw HTTP request path.
    if (span.input != null) {
      let apiMessages: Record<string, unknown>[] | undefined;
      let tools: unknown[] | undefined;

      if (Array.isArray(span.input)) {
        // Path (a): flat array already normalized by Mastra's normalizeMessages()
        apiMessages = span.input as Record<string, unknown>[];
      } else {
        // Path (b): nested request body
        ...
        if (Array.isArray(msgs)) {
          apiMessages = msgs as Record<string, unknown>[];
        }
      ...
      if (apiMessages && apiMessages.length > 0) {
        ...
        // System instructions — combine all system messages into one attribute
      ...
      // Tool definitions (only present on path (b))
    ...
    if (usage) {
      ...
      if (typeof inputDetails?.["cacheWrite"] === "number")
        otelSpan.setAttribute(
  ...
  private _convertInput(input: unknown): InputMessage[] {
    ...
    if (Array.isArray(input)) {
      ...
      for (const m of input) {
        ...
        // Standard role-based messages (AIv4 uses msg.content; AIv5 may use msg.parts)
        if (msg.role) {
          const parts =
            msg.content != null
              ? this._extractParts(msg.content)
              : Array.isArray(msg.parts)
                ? this._extractParts(msg.parts)
                : [];
          result.push({
  ...
  /** Extract plain text from a message content field (string, array of parts, or AIv5 parts). */
