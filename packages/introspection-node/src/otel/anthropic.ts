...
function setRequestAttrs(span: Span, kwargs: Record<string, unknown>): void {
  ...
  if (system) {
    ...
    if (typeof system === "string") {
      sysVal = JSON.stringify([{ type: "text", content: system }]);
    } else if (Array.isArray(system)) {
      // Anthropic block format: [{type:"text", text:"...", cache_control:{...}}]
      // Normalize to gen_ai format: [{type:"text", content:"..."}]
      sysVal = JSON.stringify(
        (system as Array<Record<string, unknown>>).map((block) => ({
          type: "text",
          content:
...
class TracedStream {
  ...
  private finalize(): void {
    ...
    if (this.cacheReadInputTokens)
      this.span.setAttribute(
        "gen_ai.usage.cache_read.input_tokens",
        this.cacheReadInputTokens,
      );
    if (this.cacheCreationInputTokens)
      this.span.setAttribute(
        "gen_ai.usage.cache_creation.input_tokens",
...
export class AnthropicInstrumentor {
  ...
  instrument(opts: {
     ...
     * Enables the Introspection processor to reliably link multi-turn API calls
     * into a single conversation without relying on history-hash matching.
