 ...
 */
export function convertMessagesToInputMessages(messages) {
    const result = [];
    for (const msg of messages) {
        const m = msg;
        if (!m || !m.role || m.role === "system")
            continue;
 ...
 */
export function extractSystemInstructions(system, messages) {
    const instructions = [];
    ...
    if (typeof system === "string" && system) {
        ...
        if (Array.isArray(system)) {
            for (const s of system) {
                ...
                if (text)
                    instructions.push({ type: "text", content: text });
            }
        }
        else {
...
export function buildOutputMessages(options) {
    ...
    if (options.reasoningText) {
        ...
        for (const r of options.reasoning) {
            if (r.text)
                parts.push({ type: "thinking", content: r.text });
        }
    }
    // Text content
    if (options.text) {
 ...
 */
export function convertToolsToToolDefinitions(tools) {
    ...
    switch (part.type) {
        ...
        case "tool-result": {
            ...
            if (typeof rawResult === "string") {
                ...
                "value" in rawResult) {
                response = JSON.stringify(rawResult.value);
            }
            else {
...
function extractTextFromSystemMessage(msg) {
...
function extractToolParameters(parameters) {
    ...
    if (p["~standard"] && typeof p["~standard"] === "object") {
        ...
        if (standardJsonSchema && typeof standardJsonSchema === "object") {
            const input = standardJsonSchema.input;
