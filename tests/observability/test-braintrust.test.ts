...
describe("Braintrust Dual Exporter Tests", () => {
  ...
  beforeEach(async () => {
    ...
    polly = setupPolly({ recordingName: "braintrust-dual-export" });

    if (
      !ensureEnvVarsForReplay(
        ["OPENAI_API_KEY", "INTROSPECTION_TOKEN", "BRAINTRUST_API_KEY"],
        "braintrust-dual-export",
      )
    ) {
      console.log(
    ...
    capture = await createBraintrustProvider();
  });
